...
async def dashboard(project_id):
    ...
    if current_user.is_authenticated:
        ...
        async with httpx.AsyncClient() as client:
            ...
            current_project             = await client.get(f"http://127.0.0.1:5000/api/get-single-project-by-users-project-id?user-id={user_id}")                                                                   # get only the project, the user is looking for to see prject details
            ...
            all_tasks_by_project        = await client.get(f"http://127.0.0.1:5000/api/get-all-task-by-username-and-project?user-id={user_id}")                                                                     # get all tasks that belongs to a user and the project the user is looking for
            all_tasks_status_toto       = await client.get(f"http://127.0.0.1:5000/api/get-all-task-by-username-group-by-project?user-id={user_id}&project-id={project_id}&status-id=todo")                         # get all tasks that belongs to a user and the project the user is looking for and groups them by status todo
            all_tasks_status_inprogress = await client.get(f"http://127.0.0.1:5000/api/get-all-task-by-username-group-by-project?user-id={user_id}&project-id={project_id}&status-id=inprogress")                         # get all tasks that belongs to a user and the project the user is looking for and groups them by status todo
            all_tasks_status_finished   = await client.get(f"http://127.0.0.1:5000/api/get-all-task-by-username-group-by-project?user-id={user_id}&project-id={project_id}&status-id=finished")                         # get all tasks that belongs to a user and the project the user is looking for and groups them by status todo
        print(user_id)
